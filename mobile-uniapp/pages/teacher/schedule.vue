<template>
  <view class="container">
    ...
    <view v-for="lesson in lessons" :key="lesson.id" class="lesson-card">
      ...
      <view class="lesson-info">班级ID: {{ lesson.classId }} | 教室ID: {{ lesson.classroomId }}</view>
...
function statusClass(s) { return {1:'s-upcoming',2:'s-done',3:'s-cancel',4:'s-moved'}[s] || '' }

async function fetchSchedule() {
  try {
    const info = uni.getStorageSync('userInfo') || {}
    const res = await uni.request({
      url: `${BASE_URL}/edu/schedules?pageNum=1&pageSize=50`,
      header: { Authorization: `Bearer ${getToken()}` }
    })
    if (res.data.code === 0) {
      lessons.value = res.data.data.records.filter(l => l.teacherId === info.userId)
    }
