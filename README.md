...
- **测试统计**：后端 425 通过 | 管理端 76 通过 | 移动端 6 通过
...
- **已知限制**：模拟支付（非真实支付网关），JWT 密钥必须通过 `JWT_SECRET` 环境变量外部注入，生产 DB 密码/JWT/CORS 由 `ProductionConfigurationValidator` 启动校验
